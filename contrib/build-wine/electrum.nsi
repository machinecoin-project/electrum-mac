...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-MAC"
  !define PRODUCT_WEB_SITE "https://github.com/machinecoin-project/electrum-mac"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-mac-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-mac\icons\electrum.ico"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mac-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mac-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-mac-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-mac-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-mac-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\machinecoin" "" "URL:machinecoin Protocol"
  WriteRegStr HKCU "Software\Classes\machinecoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\machinecoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\machinecoin\shell\open\command" "" "$\"$INSTDIR\electrum-mac-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\machinecoin"
