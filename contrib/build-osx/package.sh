...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-mac-macos
	cd /tmp/electrum-mac-macos
...
rm -rf /tmp/electrum-mac-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-mac-macos/image/
cp -r $1 /tmp/electrum-mac-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-MAC_uncompressed.dmg \
    /tmp/electrum-mac-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-MAC_uncompressed.dmg electrum-mac-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-MAC_uncompressed.dmg

echo "Done."
md5sum electrum-mac-$VERSION.dmg
